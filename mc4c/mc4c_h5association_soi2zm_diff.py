...
if not len(sys.argv) >= 3:
    print('usage:: python diff.py cellLine1.zm-soi2 cellLine2.zm-soi2 [relativeDiff=0]')
    sys.exit()
fx = [str(sys.argv[1]), str(sys.argv[2])]
rq = 0 if len(sys.argv)==3 else int(sys.argv[3])
...
# Relative difference
if rq:
    soi_dz2 = soi_dz/soi_zm[1]
    fw = open(fx[0]+'-diff2', 'w')
    ct = "#fa: %s\n" % (fx[0])
    ct+= "#fb: %s\n" % (fx[1])
    ct+= "#shape: %d fa-fb min: %+12.5e max: %+12.5e\n" % (soi_N, nanmin(soi_dz2), nanmax(soi_dz2))
    ct+= 'xxx'
    for lb in soi_lb:
        ct += "\t%s" % (lb)
    fw.write(ct+'\n')
    for i in range(soi_N):
        lt = "%s" % (soi_lb[i])
        for j in range(soi_N):
            lt += "\t%12s "%('NaN') if isnan(soi_dz2[i,j]) else "\t%+12.5e "%(soi_dz2[i,j])
        fw.write(lt+'\n')
    fw.close()
