...
if not len(sys.argv) >= 3:
    print('usage:: python mc4c_3bodyVP_summary.py q3_vp.mc4c q3_vp.hlm [lnq=0]')
    sys.exit()
#
...
for i in range(0, N):
    ...
    for j in range(0, N):
        if i==j:
            ...
            lt += "%11s "%('NaN') if isnan(cm[0][i,j]) else "%11.4e"%(cm[0][i,j]*sx)
        else:
            lt += "%11s "%('NaN') if isnan(cm[1][i,j]) else "%11.4e"%(cm[1][i,j])
    fw.write(lt+'\n')
fw.close()
